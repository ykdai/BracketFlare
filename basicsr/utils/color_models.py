...
def lab2rgb(image: torch.Tensor, clip: bool = True) -> torch.Tensor:
    ...
    L: torch.Tensor = image[..., 0, :, :]*100
    a: torch.Tensor = image[..., 1, :, :]*254-127
    _b: torch.Tensor = image[..., 2, :, :]*254-127
...
def rgb2lab(image: torch.Tensor) -> torch.Tensor:
    ...
    Returns:
        Lab version of the image with shape :math:`(*, 3, H, W)`.
        The L channel values should be in the range 0..100. a and b are in the range -127..127. 
        To make it easy to operate, the Lab channel values are set to the range 0..1.
    ...
    L: torch.Tensor = (1.16 * y) - 0.16
    a: torch.Tensor = (500.0 * (x - y) + 127)/254
    _b: torch.Tensor = (200.0 * (y - z) + 127)/254
